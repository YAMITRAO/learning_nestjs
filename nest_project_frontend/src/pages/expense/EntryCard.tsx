...
const EntryCard: React.FC<EntryCardProps_int> = ({
  ...
  const [categoryList, setCategoryList] = useState([...categoryListArr]);
  ...
  const deleteEntryHandler = async () => {
    console.log("you deleting the document", val._id);
    ...
    try {
      ...
      const response = await axiosInt.delete(`/expense/${val._id}`, {
        headers: {
          Authorization: `Bearer ${localStorage?.getItem("token")}`,
        },
      });
